...
def sensor_info():
    try:
        hum = mcp.read_adc(0)
        print ("Sensor: "+ str(hum))
        per = ((585-int(hum))*100) / 215
        # 585 = minimal humidity
        # 370 = maximal humidity or 400
        # --> 314 = difference 
        return int(per)
    except:
        return None
...
def index():
    ...
    if session == None or not session or session["id"] == None:
        ...
        return render_template("info_plant.html", plants=all_plants, days=watering_days, humidity=humidity, session=session)
...
def adding_page():
    ...
    # If logged in and reached by GET or POST
        ...
        return render_template("adding_page.html", plants=all_plants, days=watering_days, humidity=humidity, session=session)
